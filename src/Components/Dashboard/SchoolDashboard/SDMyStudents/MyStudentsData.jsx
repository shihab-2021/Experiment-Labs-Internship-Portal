...
const MyStudentsData = ({ students, setShowAddStudent }) => {
  ...
  return (
    <div className="w-11/12 mx-auto mt-4">
      ...
      <div className="flex justify-between border-b border-[#ECECEC] pb-2">
        <p className="text-[19px] text-[#797979] font-medium mt-3">
          My students
        </p>
        <img src={menu} alt="" />
      </div>
      <div className="grid grid-cols-3 justify-between gap-3 my-3">
        {displayedStudents &&
          displayedStudents?.map((student, index) => (
            <div
              key={index}
              className="border-[#D9D9D9] border-2 rounded-md py-[11px] px-2"
            >
              <div className="grid grid-flow-col justify-center gap-4">
                <div
                  className="grid items-center w-[102.55px] h-[102.55px] rounded-full text-red-50 justify-center text-3xl font-semibold"
                  style={{ backgroundColor: getRandomColor(index) }}
                >
                  {getInitials(student?.user)}
                </div>
                <div className="font-medium ">
                  <p className="text-[16.36px] font-medium tracking-wide">
                    {student?.user?.firstName} {student?.user?.lastName}
                  </p>
                  <p className="text-[#797979] text-[15.4px] font-medium tracking-wide">
                    {student?.user?.class}th Class
                  </p>

                  <p className="text-[15.4px] tracking-wide">
                    Section :{" "}
                    <span className="text-[16.4px] text-[#797979]">
                      {student?.user?.section || "Not Available"}
                    </span>
                  </p>
                  <p className="text-[15.4px] tracking-wide">
                    Phone :{" "}
                    <span
                      className="text-[16.4px] text-[#797979]"
              ...
              <p
                style={{
                  borderRadius: "3.84px",
                  background:
                    "linear-gradient(90deg, #F4F6FF 4.11%, rgba(249, 250, 255, 0.00) 99.85%)",
                }}
                className="text-[#3E4DAC] text-[15.358px] font-bold tracking-wider text-center mt-3 py-[5.759px] px-[4.799px]"
              >
              ...
              <div className="flex justify-between my-2">
                {Object.entries(
                  calculateTaskStatus(student?.taskSubmissions)
                ).map(([status, count]) => (
                  <div
                    key={status}
                  ...
                  >
                    <div
                      className={`flex gap-[6px] items-center text-[${
                        status === "rejected"
                          ? "#DD2025"
                          : status === "selected"
                          ? "#E8B912"
                          : "#F1511B"
                      }] items-center text-[19.198px] font-bold tracking-wide`}
                    >
                      <img
                        src={
                          status === "rejected"
                    ...
                    <p className="text-[#797979] font-medium text-[14.398px]">
                      {status.charAt(0).toUpperCase() + status.slice(1)} in
                      tasks
                    </p>
                  </div>
                ))}
