    ...
    if (userInfo?.organizations) {
      ...
      axios
        .get(
          ...
          }/api/v1/taskSubmissions/leaderBoard/schoolId/${
            userInfo?.organizations[0]?.schoolId
          }`
        )
        .then((res) => {
      ...
      axios
        .get(
          `${
            import.meta.env.VITE_APP_SERVER_API
          }/api/v1/schools/statisticalData/schoolId/${
            userInfo?.organizations[0]?.schoolId
          }`
        )
        .then((tasks) => {
          setSchoolTasks(tasks.data);
        })
    ...
    <div className="w-11/12 mx-auto">
      ...
      >
        <div className="flex gap-16">
          ...
          <div>
            <h1 className="my-1 text-[21px] font-semibold">
              {schoolData?.schoolName}
            </h1>
            <p className="my-1 text-[17px]  font-medium">
              Address :{" "}
              <span className="text-[#9F9F9F]">
                {schoolData?.schoolAddress || "Not Available"}
              </span>
            </p>
            <p className="my-1 text-[17px]  font-medium">
              Counsellor :{" "}
              <span className="text-[#9F9F9F]">
                {counsellorData?.orgName || "Not Available"}
              </span>
            </p>
            {/* <p className='text-[17px]  font-medium'>Phone Number</p>
                    <p className='text-[17px] text-[#9F9F9F] font-medium'>+9 8162152151</p> */}
            <p className="text-[17px] text-[#8064F0] font-medium mt-6">
              Students
            </p>
            <p className="text-[17px] text-[#9F9F9F] font-medium mb-6">
              {schoolTasks?.totalStudentsCount || 0}
            </p>
          </div>
        </div>
      ...
      <h1 className="text-[17px] font-medium text-[#3E4DAC] mt-7 mb-2">
        School dashboard{" "}
      </h1>
