...
const AdminSubUserAction = ({ subData }) => {
    ...
    useEffect(() => {
        axios
            .get(
                `${import.meta.env.VITE_APP_SERVER_API}/api/v1/tasks/organizationId/${orgId}/creatorEmail/${subuserEmail}`
            )
            .then((data) => {
                        ...
                        {item?.studentsImg.map((each, index) => (
                                ...
                                <div className="relative w-full">
                                    <div className="w-full bg-gray-200 rounded-lg h-2">
                                        <div
                                            ...
                                            style={{
                                                width: `${(item?.progressBar?.current || 0 /
                                                    item?.participantLimit) *
                                                    100
                                                    }%`,
                                            }}
                            ...
                            <div className="mt-3  grid gap-2 justify-center items-center">
                                <div className="flex gap-4 items-center my-auto text-white">
                                    <button onClick={() => updateSubmissionStatus('AdminApproved', item?._id)} className='bg-[#20B15A] py-2 px-3 rounded-3xl'>Approve</button>
                                    <button onClick={() => updateSubmissionStatus('Rejected', item?._id)} className='bg-[#DD2025] py-2 px-3 rounded-3xl'>Reject</button>
                                </div>
